...
#------------------------------------------------------------------------------
# 腳本：SSS - Minigame Slot Machine
# 【用途】保留的 Runtime 元件「SSS - Minigame Slot Machine」。
...
# 【設定／可調參數】優先調整本頁明確的設定常數／設定區，例如：SLOT_MACHINE_SHEET、SLOT_MACHINE_STOP、SLOT_MACHINE_CLICK、SLOT_MACHINE_LUCKY、SLOT_MACHINE_MISS、SLOT_ICONS。核心方法除非已確認依賴鏈，不建議直接覆寫。
# 【依賴／載入順序】含 2 個 alias／方法包裝，載入順序具有語意；登記 $imported：MinigameSlotMachine。
# 【呼叫方式／範例】未在原文件找到可證實的獨立 Script Call 範例；此頁主要由引擎或其他腳本自動呼叫。
# 【相關素材】本頁直接引用：IconSet、Open3、Switch1、Chime2、Buzzer2。刪除／改名素材前必須反查其他腳本與 Data／事件是否共用。

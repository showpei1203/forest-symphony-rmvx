...
#------------------------------------------------------------------------------
# 腳本：ATS 3.0c｜Advanced Text System
# 原作：modern algebra（rmrk.net）｜2010-09-07
# 特別致謝：Zeriab（RegExp 教學、原作者導師）；原作亦列出 Arrow-1、Irock、
# tsy0302、Kayo、deadnub 等回報／建議者。
#
# 【用途】
# ATS 3.0 是 Forest Symphony 的核心訊息系統，擴充 VX 的 Show Text／Show Choices：
# 連續訊息合併、平滑捲動、段落自動換行、進階選項、臉圖動畫、姓名框／單字框、
# 逐字速度與音效、動態視窗尺寸與角色定位、Speech Tag、Graphic Novel 顯示切換，
# 並提供大量訊息控制碼與每則訊息／全域參數 API。
#
# 【重要載入規則】
# 1. 原作要求放在 Materials 區、Main 上方，且通常不與其他大型 Message System 共存。
# 2. Forest Symphony 後方另有 ATS DialogueExtension／Quest Guide／Safety Patch；
#    目前順序已驗證，搬動前先查 LoadOrder Guide。
# 3. 本頁的 escape code、Symbol、ats_next / ats_all 方法名是 Runtime API，不可翻譯改名。
#
# 【主要功能】
# APPEND_TEXT       ：合併屬性相同的連續 Show Text。
# SCROLLING         ：超過可視行數時向上捲動；可設定速度、箭頭、自動暫停、回看、逐頁。
# PARAGRAPH_FORMAT  ：自動依視窗寬度換行；仍可用 \lb 強制換行。
# JUSTIFIED_TEXT    ：段落左右對齊。
# Advanced Choices ：可合併後續選項、條件顯示／停用、Choice Help、多行選項與獨立視窗。
# Advanced Faces   ：臉圖可鏡像、改位置／尺寸／透明度／Blend、淡入、水平／垂直滑入、動畫。
# Letter Control   ：逐字速度、等待、暫停與逐字 SE／隨機音高。
# Message Window   ：可自訂座標、寬高、WindowSkin、字體、透明度、Fit-to-text、避開角色。
# Word / Name Box  ：額外單行視窗，可獨立設定位置、字體、背景與 Dim。
# GRAPHIC_NOVEL    ：按鍵暫時隱藏 ATS 視窗，便於觀看背景圖。
# MOVE_WHEN_VISIBLE：訊息顯示時仍允許玩家移動；選項中會停用。
#
# 【通用訊息控制碼｜資料／名稱】
# \lb         強制換行
# \v[x]       變數 x 的值            \n[x]       Actor x 名稱
# \p[x]/\pid[x] 隊伍第 x 位 Actor ID  \np[x]      隊伍第 x 位 Actor 名稱
# \ni[x] / \nw[x] / \na[x] / \ns[x] / \nt[x]：Item／Weapon／Armor／Skill／State 名稱
# \nc[x] / \ne[x] / \nm[x] / \nl[x]：Class／目前地圖 Event／Enemy／Element 名稱
# \nv[x] / \nsw[x]：Variable／Switch 名稱
# \map / \map[x]：目前地圖／指定 Map ID 名稱
# \di[x] / \dw[x] / \da[x] / \ds[x]：Item／Weapon／Armor／Skill 說明
# \pi[x] / \pw[x] / \pa[x]：價格
# \i#[x] / \w#[x] / \a#[x]：隊伍持有數量
# \ac[x]：Actor 職業
#
# 【通用訊息控制碼｜圖示／文字樣式】
# \i[x]：直接畫 Icon Index；\ii / \wi / \ai / \si / \ti：依資料 ID 畫對應圖示。
# \c[x]：WindowSkin palette 顏色；\c[#RRGGBB]：十六進位 RGB 顏色。
# \fn[font]：字體；\fs[x]：字體大小；\fa[x]：文字透明度。
# \b / /b：粗體開／關；\i / /i：斜體；\s / /s：陰影；\u / /u：底線。
# \hl[x]：反白；/hl 或 \hl[-1]：關閉反白。
# \l / \r / \c：左／右／置中；\t：Tab；\x[n]：直接設定目前繪製 X。
# \f[key]：讀取 FILTERS[key]。
# \s[x,text]：Switch x ON 才畫 text；\s![x,text]：Switch x OFF 才畫。
#
# 【資料方法控制碼】
# \vocab[method]       ：顯示 Vocab.method 回傳值。
# \actor_method[x]     ：對 Actor x 呼叫 method。
# \i_method[x]         ：對 RPG::Item x 呼叫 method。
# \w_method[x]         ：對 RPG::Weapon x 呼叫 method。
# \a_method[x]         ：對 RPG::Armor x 呼叫 method。
# \s_method[x]         ：對 RPG::Skill x 呼叫 method。
# \t_method[x]         ：對 RPG::State x 呼叫 method。
# \enemy_method[x]     ：對 RPG::Enemy x 呼叫 method。
# 上述資料物件的 note 會回傳完整備註欄；n 僅回傳 \msg[text]msg/ 包住的內容。
# \#{ruby_code}#       ：執行 Ruby 並繪製回傳值；僅在確定程式碼安全時使用。
#
# 【Message Window 專用控制碼】
# \g：開／關 Gold Window。       \nb[name] / /nb：開／關姓名框。
# \wb[word] / /wb：開／關單字框。
# \. 等待 15 frame；\| 等待 60 frame；\w[x] 等待 x frame。
# \! 暫停等待輸入；\^ 跳過下一個 Pause。
# \> 加快一級；\< 放慢一級。
# \S[x] 以 x 調整逐字等待時間；\S[=x] 直接指定等待時間，例如 \S[=0] 每字 1 frame，
# \S[=-1] 為立即顯示。
# \se[name]：播放 SE；\me[name]：播放 ME。
# \ani[target_id, animation_id]：對目標播放動畫；0=Player，正數=Event ID。
# \bln[target_id, balloon_id]：播放 Balloon。
# \af[x]：使用 Actor x 的臉圖。
# \pb：強制新頁。
# \oc[x] / \uc[x] / \lc[x] / \rc[x]：訊息窗位於角色上／下／左／右。
# \e[x]：優先在角色上方，空間不足時改下方。
# \mxy[x,y] / \fxy[x,y] / \nxy[x,y]：直接指定訊息／臉圖／姓名框位置。
#
# 【選項專用控制碼】
# \skip       ：游標略過，適合標題或空白行。
# \soff[x]    ：Switch x OFF 才顯示選項。
# \son[x]     ：Switch x ON 才顯示選項。
# \d[x]       ：Switch x OFF 時使選項不可選。
# \d![x]      ：Switch x ON 時使選項不可選。
# \wb[text]   ：此選項被游標指向時，在 Choice Help 顯示 text。
# \+{text}    ：放在 Choice Branch 對應 when 下方的 Comment，可把 text 追加至選項，
#               用來突破 VX 原生選項字數限制並製作多行選項。
#
# 【每則訊息／全域 API】
# ats_next(:property, value)      # 只影響下一則訊息
# $game_message.property = value # 同上，直接設定下一則訊息狀態
# ats_all(:property, value)       # 修改全域預設，並同步目前訊息
# $game_ats.property = value     # 修改全域 ATS 預設
# $game_ats.reset                # 恢復 Game_ATS 常數預設
# 使用 ats_next / ats_all 時 property 要保留前面的冒號；直接呼叫 accessor 則不加冒號。
#
# 【重要 Property 分組】
# 訊息：:max_lines、:message_speed、:skip_disabled、:append_text、:scrolling、
# 主視窗：:message_x / :message_y / :wlh / :battle_wlh / :do_not_obscure /
# 臉圖：:face_x / :face_y / :face_z / :face_side / :face_offset_x / :face_offset_y /
#       :face_scroll_speed / :animate_faces / :letters_per_face / :face_window 等。
# 選項：:choice_text / :disabled_choice_text / :choicebox_text / :choice_window /
# Choice Help、Name Box、Word Box 亦各自有 X/Y/width/height/opacity/backopacity/
# windowskin/font/wlh/dim/use_dim 等同名設定。
#
# 【ats_next 專用】
# :do_not_refresh：true 時後續訊息即使中間有 Show Picture／Control Variables 等事件也
#                 不清空既有文字；最後一則前要 ats_next(:do_not_refresh, false)。
# :character     ：預先指定相對定位角色；0=Player，正數=Event ID。
# :char_ref      ：0=上、1=左、2=下、3=右、4=上方放不下則下方。
#
# 【目前 Forest Symphony 主要預設】
# MAX_LINES=4、MESSAGE_SPEED=1、APPEND_TEXT=false、APPEND_CHOICE=true、
# SCROLLING=false、PARAGRAPH_FORMAT=false、MOVE_WHEN_VISIBLE=false、
# GRAPHIC_NOVEL=false；字體主要使用「微軟正黑體」。詳細數值直接看下方 Game_ATS
# EDITABLE REGION，該區每個常數旁已改為繁中說明。
#
# 【素材】
# WORD_WINDOWSKIN 等讀取 Graphics/System；MESSAGE_DIM 等使用 MessageBack；
# SPEECH_TAG_GRAPHICS 使用 Window_Popup、Speech Tag 2、Thought Tag 1；音效名稱由各 SE
# 常數決定。刪除素材前要反查 FS ATS 擴充與事件。
#
# 【來源／授權】
# 原作者 modern algebra（rmrk.net）。本中文整理保留所有 Runtime 控制碼與 API 原字串。
#==============================================================================
#==============================================================================
# 【Phase 32a｜ATS Runtime 修復】
#------------------------------------------------------------------------------
# Phase 15 的註解中文化工具曾把 Regex／字串插值中的 # 誤判為 Ruby 註解起點，
# 造成 perform_substitution、perform_conversion 與三段 eval setter 被截斷。
# 本版保留上方繁中完整手冊，但自 Game_ATS 起恢復 Phase 14 翻譯前已驗證的
# 原始 Runtime 本體；後續若再次中文化內文，必須使用 Ruby-aware lexer，不得以 # 猜註解。
#==============================================================================
#==============================================================================
# ** Game_ATS
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
class Game_ATS
  ...
  SPEECH_TAG_INDEX = 0         # See line 418.
  # SPEECH_TAG_GRAPHICS         # See line 422.
    SPEECH_TAG_GRAPHICS = ["Window_Popup", "Speech Tag 2", "Thought Tag 1"]
  ...
  MESSAGE_WIDTH = 544#450           # See line 491.
  ...
  MESSAGE_FONTNAME = ["微軟正黑體", "Arial", "Courier New"] # See line 503.
  ...
  FACE_SIDE = false              # See line 520.
  FACE_OFFSET_X = -20             # See line 522.
  FACE_OFFSET_Y = 0             # See line 524.
  FACE_WIDTH = 0                # See line 526.
  FACE_HEIGHT = 0               # See line 529.
  FACE_MIRROR = true           # See line 532.
  ...
  CHOICE_WIDTH = 192#192            # See line 591.
  CHOICE_HEIGHT = -1            # See line 594.
  COLUMN_MAX = 1                # See line 596.
  ROW_MAX = 3                   # See line 597.
  ...
  CHOICE_FONTNAME = ["微軟正黑體"] # See line 607.
  ...
  NAME_OFFSET_X = 50            # See line 666.
  NAME_OFFSET_Y = 16+5            # See line 668.
  NAME_OPACITY = 0            # See line 670.
  ...
  NAME_FONTCOLOUR = 1           # See line 679.
  NAME_FONTNAME = ["微軟正黑體", "Arial", "Courier New"] # See line 682.
  NAME_FONTSIZE = 22            # See line 686.
  NAME_DIM = "MessageBack"      # See line 688.
  NAME_USE_DIM = 0              # See line 690.
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  WORD_FONTNAME = ["微軟正黑體", "Arial", "Courier New"] # See line 718.
